  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      const response = await Api.post(
        ...
        {
          responseType: "blob", // Specifies that the response is a blob
        }
      );
      ...
      if (response.status === 200) {
        ...
        const errorBlob = response.data; // Convert the binary to response
        const errorText = await errorBlob.text(); // Read the binary as text
        setMessage(`Error: ${errorText || "An unexpected error occurred."}`);
      }
    } catch (error) {
      const errorMessage = error.response?.data
        ? await error.response.data.text() // Try to read as text if it's a Blob
        : error.message;
      setMessage(`Error: ${errorMessage || "An unexpected error occurred."}`);
    }
  }, [dateFrom, dateTo]);
...
// you can add more css as i am not good with css. also pls help me remove the api you console.log in your api.config file
